...
The results reveal that all predictors are significant. This means that the chance of passing the Bechdel test is 8-fold if the entire writer team is female, compared to an all male writer team. This holds even if we account for the (small) effects of the release decade (newer movies pass the test more often), and runtime (longer movies are  more likely to fail the test, wtf?).
...
In general, `r percent(mean(movies$bechdel_pass))` of the investigated movies passed the Bechdel test.  

```{r}

augment(bechdel_model, type.predict = "link") %>% 
    ggplot() +
    ...
    labs(x = "% of female writers for the movie",
         ...
         subtitle = "When all writers are female, passing the Bechdel test is 5 times more likely compared to an all male writer team.")
```
