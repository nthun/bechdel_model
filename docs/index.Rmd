...
This is a tutorial to use R for creating an analysis report
...
This dataset is collected by FiveThirtyEight.
...
There are two datasets in the project. The first one contains `r nrow(raw_bechdel)` Bechdel ratings, movie title, and release year. Let's just make two quick plots to see the distribution of the test results and the distribution by year.
...
Finally, I create the final analysis dataset, where I only keep relevant variables, recode the outcome to numeric (0/1), create a decade variable instead of year, and convert the runtime to a numeric value.
...
bechdel_model <-
    glm(bechdel_pass ~ female_writer + decade + runtime, data = movies, family = "binomial")
...
# This is for conveniently making an APA compatible summary table.
tab_model(bechdel_model, show.aic = TRUE)

```

The results reveal that all predictors are significant. This means that the chance of passing the Bechdel test is 8-fold if the entire writer team is female, compared to an all male writer team. This holds even if we account for the (small) effects of the release decade (newer movies pass the test more often), and runtime (longer movies are slightly more likely to fail the test, wtf?).
...
In general, `r percent(mean(movies$bechdel_pass))` of the investigated movies passed the Bechdel test. This 

```{r}

augment(bechdel_model) %>% 
    ggplot() +
    aes(x = female_writer, y = exp(.fitted)) +
    geom_smooth() +
    ...
    labs(x = "% of female writers for the movie",
         ...
         title = "The association of proportion of female writers and passing the Bechdel test in movies",
         subtitle = "When all writers are female, passing the Bechdel test is 5 times more likely. For all male writer team, ")
```
